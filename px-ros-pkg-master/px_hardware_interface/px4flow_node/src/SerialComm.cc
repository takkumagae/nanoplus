...
SerialComm::open(const std::string& portStr, int baudrate)
{
    ...
    syncTimer = nh.createTimer(ros::Duration(2.0), boost::bind(&SerialComm::syncCallback, this, _1));

    m_connected = true;
//    mavlink_msg_param_value_send(MAVLINK_COMM_1,"IMAGE_L_LOW",1,MAVLINK_TYPE_FLOAT,1,1);
...
SerialComm::readCallback(const boost::system::error_code& error, size_t bytesTransferred)
{
    ...
    if (msgReceived)
    {
        ...
        case MAVLINK_MSG_ID_OPTICAL_FLOW:
        {
            ...
            optFlowMsg.velocity_x = flow.flow_comp_m_x*(-sqrt(2)/2)+flow.flow_comp_m_y*(-sqrt(2)/2);
            optFlowMsg.velocity_y = flow.flow_comp_m_x*(-sqrt(2)/2)+flow.flow_comp_m_y*(sqrt(2)/2);
...
SerialComm::syncCallback(const ros::TimerEvent& timerEvent)
{
  if (m_systemId == -1)
    ...
    static bool low_light=false;
    if(!low_light)
    {
      mavlink_msg_param_set_pack(0, 0, &msg, m_systemId, m_compId, "IMAGE_L_LIGHT", 1, MAVLINK_TYPE_FLOAT);
      size_t messageLength = mavlink_msg_to_send_buffer(m_buffer, &msg);
      if (messageLength != boost::asio::write(m_port, boost::asio::buffer(m_buffer, messageLength)))
      {
        ROS_WARN("Unable to set low light mode over serial port.");
      }
      else 
      {
        low_light = true;
        ROS_INFO("set low light successfully.");
      }
